...
describes what users get. Pick a format with `-o`, or let geo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
geo <command> -o table   # aligned columns for reading
geo <command> -o jsonl   # one JSON object per line, for piping
geo <command> -o json    # a single JSON array
geo <command> -o csv     # spreadsheet friendly
geo <command> -o tsv     # tab-separated
geo <command> -o url     # just the URL column
geo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
geo <command> --fields id,title,url
```
...
```bash
geo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
geo <command>            # a table, because this is a terminal
geo <command> | wc -l    # JSONL, because this is a pipe
```
